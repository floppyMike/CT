...
public:
	...
	void spawn(DNode* from) noexcept
	{
		m_selected = std::make_unique<DLink>(m_r,
			sdl::Line{ { from->shape().x + 2, from->shape().y + 2 }, mousePosition() });
	...
	{
		m_selected->shape({ m_selected->shape().pos1(), { to->shape().pos() + sdl::Point(2, 2) } }).toNode(to);
	...
	void translateEnd(const sdl::Point<int>& delta)
	{
