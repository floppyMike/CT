...
public:
		...
		, m_dur(std::chrono::steady_clock::now() + DURATION)
	{
		*pthis->m_doRender = true;
		...
		for (auto &i : *m_iter) i.flipTo(RoadLights::TrafficState::PASSING);
	}

	void update() override
	{
		for (auto &i : pthis->m_roads) i->update();

		if (std::chrono::steady_clock::now() >= m_dur)
		{
			auto nextSeq = _nextIter_();

			for (auto [iterPrev, iterNew] = std::pair(m_iter->begin(), nextSeq->begin()); iterPrev != m_iter->end();
				 ++iterPrev)
			{
				if (iterPrev->traffic_node() == iterNew->traffic_node() && iterPrev->state() != iterNew->state())
				{
					iterNew->flip();
					++iterNew;
				}
	...
	void draw() override
	{
		for (auto &i : pthis->m_roads) i->draw();

		for (auto &i : *m_iter)
		{
			pthis->m_rend.color(sdl::RED);
...
private:
	...
	std::vector<RoadLightsState> m_states; 
